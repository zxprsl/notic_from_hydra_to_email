...
LEFT JOIN names n ON d.N_SUBJECT_ID = n.N_SUBJECT_ID
 WHERE     TRUNC(D.D_END + 1/24/60/60) = TRUNC(SYSDATE) + :day_shift -- В N указать точное количество дней, через которое срок действия оборудования закончится (Указание точной даты)
--WHERE     (D.D_END > SYSDATE AND D.D_END <= TRUNC(SYSDATE) + N) -- В N указать количество дней, для того, чтобы определить интервал от текущей даты, в который должна попасть дата завершения срока действия оборудования
ORDER BY D.D_END
