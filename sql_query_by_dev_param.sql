--SQL-запрос для поиска оборудования, у которого через N дней закончится срок действия дополнительного параметра "Дата окончания БЗ"
WITH OBJECTS AS (       SELECT  SS.N_SUBJECT_ID,
                                SS.N_OBJECT_ID,
                                SI_OBJECTS_PKG_S.GET_NAME_BY_ID(SS.N_OBJECT_ID) VC_NAME,
                                OV.D_VALUE OBJECT_END,
                        ...
                        FROM    SI_V_OBJ_SUBJECTS   SS,
                                ...
                                SI_V_USERS          JR
                        WHERE   1=1
                        AND     SS.N_OBJECT_ID = OV.N_OBJECT_ID
                        AND     SS.N_SUBJECT_ID = JR.N_SUBJECT_ID
                        AND     OV.N_GOOD_VALUE_TYPE_ID IN (4849932501,4849932601) -- Дополнительгный параметр "Дата окончания БЗ"
                        AND     OV.D_VALUE >= SYSDATE -- На оборудовании есть дата окончания адреса, причем позднее, чем текущая дата
                        AND     OV.VC_VISUAL_VALUE IS NOT NULL
            ),
